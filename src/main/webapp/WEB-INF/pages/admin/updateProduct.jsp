...
<body>
	<div class="add-product-form">
		...
		<h2>Edit Product</h2>
		<form method="post" action="admin-update-save?id=${product.id}"
			...
			<div class="form-group">
				<label for="category">Category</label> <select name="categoryId"
					...
					<c:forEach var="cat" items="${categories}">
						...
						<option value="${cat.id}"
							${cat.id == product.category ? 'selected' : ''}>${cat.name}</option>
			...
			<div class="form-group">
				<label for="imageUrl">Choose An Image</label>
				<div>
					<img id="imagePreview" src="${product.imageURL}"
						alt="Current Image"
						style="max-width: 200px; display: block; margin-bottom: 10px;" />
				</div>
				<input type="file" name="imageFile" value="${product.name}" required />
			</div>
